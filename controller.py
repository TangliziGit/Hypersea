...
class Controller(nn.Module):
    def __init__(self, in_dim, out_dim, filter_height, filter_width, stride_height, stride_width):
        ...
        #     nn.Conv2d(in_dim, 70, kernel_size=(3, 14), stride=(1, 1)),
        #     nn.Conv2d(70, out_dim, kernel_size=(filter_height, filter_width), stride=(stride_height, stride_width)),
        #     nn.MaxPool2d((2, 2), stride=(2, 2))
        # )
        # # self.fc1 = nn.Linear(10 * 15 * 28, 10)
        # self.fc1 = nn.Linear(
        #     # 32 X 32 的经过第一层卷积之后 变成了  30 X 19 的
        #     out_dim * int(((30 - filter_height) / stride_height + 1) / 2) * int(
        #         ((19 - filter_width) / stride_width + 1) / 2), 10)

        # self.layer = nn.Sequential(
        #     #nn.Conv2d(in_dim, out_dim, kernel_size=(filter_height, filter_width), stride=(stride_height, stride_width)),
        #     nn.Conv2d(in_dim, 50, kernel_size=(4, 5), stride=(3, 2)),
        #     nn.Conv2d(50, 70, kernel_size=(1, 2), stride=(1, 1)),
        #     nn.Conv2d(70, out_dim, kernel_size=(filter_height, filter_width), stride=(stride_height, stride_width)),
        #     nn.MaxPool2d((2, 2), stride=(2, 2))
        # )
        # # self.fc1 = nn.Linear(10 * 15 * 28, 10)
        # self.fc1 = nn.Linear(
        #     # 32 X 32 的经过第一层卷积之后 变成了  30 X 19 的
        #     out_dim * int(((10 - filter_height) / stride_height + 1) / 2) * int(
        #         ((13 - filter_width) / stride_width + 1) / 2), 10)
        ...
        self.fc1 = nn.Linear(out_dim * int(((14 - filter_height) / stride_height + 1) / 2) * int(((24 - filter_width) / stride_width + 1) / 2), 100)
    ...
    def forward(self,inputx):
        ...
        y = self.layer(inputx)
        y1 = y.view(-1, self.out_dim * int(((14 - self.filter_height) / self.stride_height + 1) / 2) * int(((24 - self.filter_width) / self.stride_width + 1) / 2))
        y2 = self.fc1(y1)
        y3 = self.dropout(y2)
        y4 = self.fc2(y3)
        return y4
    ...
    def attention(self): #一步注意力机制，从Config中获取v_t与h_t,经过注意力机制后输出z_t送予LSTM，将a_t存储在Config中
        ...
        Config.a_t = a_t  # [1*5]
        v_t = torch.tensor([[Config.of_filter, Config.filter_height, Config.filter_width, Config.stride_height, Config.stride_width]]).to(self.DEVICE).float()
